...
function server_listening() {
    console.log('Un cliente se creo escuchando en el puerto ' + client_port);
    connect();
}

function connect() {
    console.log('Estableciendo conexion con el servidor ' + address + '.');
    repeat_call();
}
...
app.get('/proximo_bus', function(req, res) {
    console.log('Proximo Colectivo: '+ req.query.next_bus+' sec.');
});
